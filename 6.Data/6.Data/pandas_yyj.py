...
def fill_state_code(row):
	if row['state'] != 0:
		state = process.extractOne(row['state'], states,score_cutoff=80)
		if state:
			state_code = state_to_code[state[0]]
			return state_code
...
if __name__ =='__main__':
	...
	# print fuzz.ratio('yangyujie','yang yu jie')
	# print process.extract('Mississippi', states)
	# print process.extractBests('Mississippi', states,limit=1)
	...
	#写入文件
	data.to_excel('sales_result.xls',sheetname='result',index=False)
